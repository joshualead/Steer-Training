const QUERIES = {
  INSERT_ADMIN_TABLE: "INSERT INTO admin_table (user_id, name) VALUES (?,?)",
  ...
  UPDATE_TRAINER_ASSIGNED_COURSE:
    "UPDATE trainer_table SET course_id = NULL WHERE user_id = ?;",
  UPDATE_ASSIGN_STATUS:
    "UPDATE course_table SET assigned_status = ?  WHERE course_id = ?;",
  UPDATE_ASSIGNED_COURSE:
    "UPDATE trainer_table SET course_id = ?  WHERE user_id = ?;",
  ...
  GET_COURSE_STATUS:
    "SELECT assigned_status FROM course_table WHERE course_id = ?",
  ...
  CHECK_IF_COURSE_IN_TCMAP: "SELECT DISTINCT(course_id) FROM tcmap_table;",
